...
export class StudentParentModalComponent {
  ...
  private loadDropdownOptions(): void {
    ...
    forkJoin({
      ...
      next: ({ students, parents }) => {
        this.studentOptions = (students.data ?? []).map(
          (student: any) => ({
            value: student.id,
            label:
        ...
        this.parentOptions = (parents.data ?? []).map(
          (parent: any) => ({
            value: parent.id,
            label:
      ...
      error: (err) => {
        ...
        this.toast.error(
          'Error',
          'Failed to load student/parent list'
        );
      },
    });
  }
  ...
  private submitUpdateAction(): void {
    ...
    this.studentParentService
      .updateStudentParent(this.currentID, payload)
      .subscribe({
        next: (res) => {
          this.toast.success('Success', res.message);
    ...
    this.studentParentService
      ...
      .subscribe({
        next: (response) => {
          ...
          this.parent_name =
            `${data.parent?.first_name ?? ''} ${data.parent?.last_name ?? ''}`.trim();
        ...
        error: (err) => {
          const msg =
            err?.error?.message ??
            'Failed to load student-parent details.';
